...
print('-------------------- PART B -----------------------------------')
for m in range(2,5):
  mask, centers = K_Means(data, m)
  for j in range(len(centers)):
    summ = 0
    for i in range(len(data)):
      if mask[i] == j:
        summ += distance(data[i], centers[j])
    print('Number of K = {} ==> Error for Center {} : {}'.format(m, j+1, summ))
  if m != 4:
    print('---------------------------------------------------------------')
print('---------------------------------------------------------------')
  ...
  for j in range(len(centers)):
    ...
    for i in range(len(data)):
      if mask[i] == j:
        summ += distance(data[i], centers[j])
    sumOfErrors += summ
  print('Number of K = {} ==> Error {}'.format(m, sumOfErrors))
  if m != 4:
    print('---------------------------------------------------------------')
print('-------------------- PART D -----------------------------------')
  ...
  for j in range(len(centers)):
    ...
    for i in range(len(data)):
      if mask[i] == j:
        summ += distance(data[i], centers[j])
    sumOfErrors += summ
  kErrors.append(sumOfErrors)
for i in range(1,10):
  xi.append(i)
plt.plot(xi, kErrors)
plt.show()
print('---------------------------------------------------------------')
for j in range(len(kErrors)):
  print('for K = {} cluster error is : {}'.format(j + 1, kErrors[j]))
